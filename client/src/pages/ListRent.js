...
import FiltersRent from "../components/FiltersRent";
...
function ListRent() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            await axios.get(`http://localhost:8080/properties?location=${searchParams.get('location')}&saleOrRent=to-rent&beds=${searchParams.get('beds')}&price=${searchParams.get('price')}&type=${searchParams.get('type')}`)
    ...
    /***** BANNER *****/
    let banner;
    if (searchParams.get('location')) { banner = <h1>Property to rent in {searchParams.get('location')}</h1> }

    return (
        <div>
            <FiltersRent location={location} />
